...
pub fn addHotModule(
    ...
    return addWorkModule(b, name, root_source_file, options, .dynamic);
}
...
pub fn addStaticModule(
    ...
    return addWorkModule(b, name, root_source_file, options, .static);
}
...
pub fn makeTracyOptions(b: *std.Build) TracyOptions {
    return .{
        ...
        .callstack_depth = b.option(i32, "tracy-callstack-depth", "Tracy callstack depth") orelse 0,
        .on_demand = b.option(bool, "tracy-on-demand", "Enable Tracy on-demand mode") orelse false,
        .only_localhost = b.option(bool, "tracy-only-localhost", "Bind Tracy to localhost only") orelse true,
        .delayed_init = b.option(bool, "tracy-delayed-init", "Enable Tracy delayed init") orelse true,
        .manual_lifetime = b.option(bool, "tracy-manual-lifetime", "Enable Tracy manual lifetime") orelse true,
    };
}
...
pub fn applyTracyBuildMode(b: *std.Build, tracy: TracyOptions) void {
    if (!tracy.enable) return;

    if (@hasField(std.Build.Graph, "incremental")) {
        b.graph.incremental = false;
    }
}

pub fn addBuildOptionsModule(
    b: *std.Build,
    tracy: TracyOptions,
    hot_reload_enable: bool,
) *std.Build.Step.Options {
    const options = b.addOptions();
    options.addOption(bool, "hot_reload_enable", hot_reload_enable);
...
pub fn attachBuildOptions(compile: *std.Build.Step.Compile, options: *std.Build.Step.Options) void {
